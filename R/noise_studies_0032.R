...
iteration_id <- "noise_studies_0032"
seed_num <- 4878232
...
grid <- grid %>%
  mutate(
    colour1 = sample(
      c("#331E36", "#41337A", "#6EA4BF", "#C2EFEB"), n(), replace = TRUE),
    colour2 = sample(
      c("#FAA275", "#FF8C61", "#CE6A85", "#985277", "#5C374C"), n(), replace = TRUE))
...
ggplot() +
  geom_curve(
    ...
    aes(x = x, y = y_warped, xend = y, yend = x_warped),
