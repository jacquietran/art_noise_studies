...
iteration_id <- "noise_studies_0047"
seed_num <- 4878247
...
colour_vec1 <- c("#392E37", "#4A3D3D", "#776B55")
colour_vec2 <- c("#DCCB93", "#D2BC74", "#C8AD55")
outline_colour <- "#A9924C"
...
ggplot() +
  geom_curve(
    data = grid,
    aes(x = x, y = y, xend = x_warped, yend = y_warped, size = size * 1.5,
        colour = colour1),
    curvature = -0.2) +
  geom_point(
  ...
  geom_point(
    data = grid %>% filter(subset < 6 & subset >= 4),
    aes(x = x_warped, y = y_warped, size = size / 2),
    colour = bg_colour, shape = 5) +
  geom_point(
    data = grid %>% filter(subset < 4),
    aes(x = x_warped, y = y_warped, size = size * 0.5, colour = colour2),
    shape = 16) +
  geom_path(
    data = grid %>% filter(subset >= 10),
