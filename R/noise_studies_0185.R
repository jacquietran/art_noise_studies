...
iteration_id <- "noise_studies_0185"
seed_num <- 568185
initial_grid_size <- 30
warp_factor <- 70 # lower = more warping
colour_vec1 <- c("#99150B", "#FF740A", "#80C6B9", "#3D8C92", "#EAE7CF") # 5 cols
bg_colour <- "#EAE7CF"
...
ggplot() +
  geom_curve(
    data = grid,
    aes(x = x, y = y, xend = x_warped, yend = y_warped, size = size * 15,
        colour = colour),
    curvature = -0.1) +
  geom_point(
