...
iteration_id <- "noise_studies_0109"
seed_num <- 4587109
initial_grid_size <- 6
...
colour_vec1 <- c("#E56B6F", "#E77C76", "#E88C7D", "#EAAC8B", "#D06125") # 5 cols
colour_vec2 <- c("#283C55", "#355070", "#6D597A", "#915F78")
outline_colour <- "#B56576"
...
ggplot() +
  ...
  ggfx::with_blend(
    geom_point(
      ...
      aes(x = x_warped, y = y_warped, size = size * 0.6, colour = colour2),
      shape = 16),
    bg_layer = "base",
  ...
  geom_point(
    data = grid %>% filter(subset > 15),
    aes(x = x_warped, y = y_warped, size = size * 0.25),
