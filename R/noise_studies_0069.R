...
iteration_id <- "noise_studies_0069"
seed_num <- 215769
...
colour_vec1 <- c("#0450B4", "#046DC8", "#1184A7", "#15A2A2", "#6FB1A0") # 5 cols
colour_vec2 <- c("#B4418E", "#D94A8C", "#EA515F", "#FE7434", "#FEA802")
outline_colour <- "#FEA802"
...
ggplot() +
  ...
  geom_point(
    ...
    aes(x = x_warped, y = y_warped, size = size * 0.5),
    colour = line_colour, shape = 0) +
  geom_point(
    data = grid %>% filter(subset < 4),
    aes(x = x_warped, y = y_warped, size = size * 0.7, colour = colour2),
    shape = 15) +
  geom_path(
    data = grid %>% filter(subset >= 10),
