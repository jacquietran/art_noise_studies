...
iteration_id <- "noise_studies_0140"
seed_num <- 789140
initial_grid_size <- 30
warp_factor <- 80 # lower = more warping
line_colour <- "#BBACC1"
bg_colour <- "#E7E3DA"

colour_vec1 <- c(
  paletteer::paletteer_d("ochRe::lorikeet")[1],
  "#54783C",
  paletteer::paletteer_d("ochRe::lorikeet")[6],
  "#84A830",
  paletteer::paletteer_d("ochRe::lorikeet")[5]) # 5 cols
colour_vec2 <- c(
  paletteer::paletteer_d("ochRe::lorikeet")[2],
  paletteer::paletteer_d("ochRe::lorikeet")[3],
  paletteer::paletteer_d("ochRe::lorikeet")[4])
outline_colour <- "#BBACC1"
...
ggplot() +
  ...
  ggfx::with_blend(
    ...
    blend_type = "overlay") +
  geom_point(
    data = grid %>% filter(subset < 10 & subset >= 7),
